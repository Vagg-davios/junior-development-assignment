...
<div class="flex flex-col items-center bg-white  rounded-lg shadow md:flex-row  ">
    ...
    <div class="flex flex-col justify-between p-4 ml-4 leading-normal">
        ...
        <form method="POST" action="./borrow">
            ...
            <input type="hidden" name="book_id" value=<?= $book["book_id"] ?>> <!-- No need to set it here since we want POST. Just for demonstration purposes-->
            <input type="hidden" name="user_id" value=<?= $_SESSION["user"] ?>> <!-- No need to set it here since we want POST. Just for demonstration purposes-->
            <input type="hidden" name="due_date" value=<?= date("Y-m-d", strtotime(date("Y-m-d") . "+1 month")) ?>> <!-- No need to set it here since we want POST. Just for demonstration purposes-->
            <button type="submit" class="text-white  mr-40 bg-[#FF9119] hover:bg-[#FF9119]/80 focus:ring-4 focus:outline-none focus:ring-[#FF9119]/50 font-medium rounded-lg text-base px-5 py-2.5 text-center inline-flex items-center dark:hover:bg-[#FF9119]/80 dark:focus:ring-[#FF9119]/40 mr-2 mb-2">
